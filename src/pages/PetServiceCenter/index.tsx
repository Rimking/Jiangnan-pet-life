  ...
  const followUpActions = [
    {
      ...
      accent: '#5A78D4',
      onClick: () =>
        activePetId ? Taro.navigateTo({ url: `/pages/AddPetReminder/index?petId=${activePetId}` }) : undefined,
    },
    {
      title: '新增记录',
    ...
    },
    {
      ...
      accent: '#7A61A7',
      onClick: () =>
        activePetId ? Taro.navigateTo({ url: `/pages/PetTimeline/index?petId=${activePetId}` }) : undefined,
    },
  ];
    ...
    >
      <View className="px-6 pt-4 pb-[110rpx]">
        ...
        <View className="rounded-[28rpx] bg-[#FFF7D5] px-5 py-6 mb-5 shadow-[0_16rpx_34rpx_rgba(230,193,82,0.18)]">
          ...
          <Text className="text-[22rpx] text-[#7D6532] leading-[1.7] mt-[10rpx] block">{heroDesc}</Text>
        ...
        <View className="grid grid-cols-1 gap-3 mb-5">
          {serviceCards.map((item) => (
            ...
            >
              ...
              <Text className="text-[42rpx] font-semibold text-[#2C2C2C] mt-[8rpx] block">{item.value}</Text>
              <Text className="text-[22rpx] text-[#666] mt-[6rpx] leading-[1.5]">{item.subtitle}</Text>
        ...
        {activePet ? (
          <View className="rounded-[24rpx] bg-white p-5 mb-5 shadow-[0_14rpx_28rpx_rgba(0,0,0,0.06)]">
            <Text className="text-[28rpx] font-semibold text-[#2C2C2C] block">{activePet.name} 的当前照护面板</Text>
            ...
            <View className="grid grid-cols-3 gap-3 mt-4">
              <View className="rounded-[18rpx] bg-[#EEF4FF] p-4">
                ...
                <Text className="text-[34rpx] font-semibold text-[#2C2C2C] mt-[6rpx]">{dashboard?.quickStats.expenseCount || 0}</Text>
              </View>
              <View className="rounded-[18rpx] bg-[#FFF7E5] p-4">
                <Text className="text-[22rpx] text-[#8A6A2C]">食物档案</Text>
                <Text className="text-[34rpx] font-semibold text-[#2C2C2C] mt-[6rpx]">{dashboard?.quickStats.foodCount || 0}</Text>
              </View>
              <View className="rounded-[18rpx] bg-[#F5F0FF] p-4">
                <Text className="text-[22rpx] text-[#7A61A7]">用药档案</Text>
                <Text className="text-[34rpx] font-semibold text-[#2C2C2C] mt-[6rpx]">{dashboard?.quickStats.medicineCount || 0}</Text>
              </View>
            </View>
            <View className="grid grid-cols-1 gap-3 mt-4">
              {followUpActions.map((item) => (
                <View key={item.title} className="rounded-[18rpx] bg-[#F8F8F8] p-4" onClick={item.onClick}>
                  <Text className="text-[24rpx] font-semibold text-[#2C2C2C]">{item.title}</Text>
                  <Text className="text-[22rpx] text-[#666] mt-[8rpx] block leading-[1.5]">{item.subtitle}</Text>
        ...
        <View className="grid grid-cols-2 gap-3 mb-5">
          ...
          >
            ...
            <Text className="text-[22rpx] text-[#666] mt-[8rpx] block">还有 {dashboard?.totals.pendingReminderCount || 0} 条待办</Text>
          ...
          >
            ...
            <Text className="text-[22rpx] text-[#666] mt-[8rpx] block">低库存食物 {dashboard?.totals.lowInventoryCount || 0} 条</Text>
          ...
          >
            ...
            <Text className="text-[22rpx] text-[#666] mt-[8rpx] block">临近结束用药 {dashboard?.totals.dueMedicineCount || 0} 条</Text>
        ...
        {dashboard?.latestMilestone ? (
          ...
          >
            ...
            <Text className="text-[24rpx] text-[#44313C] mt-[10rpx] block">{dashboard.latestMilestone.title}</Text>
            <Text className="text-[22rpx] text-[#8A6A7B] mt-[6rpx] block">{dashboard.latestMilestone.occurredAt.slice(0, 10)}</Text>
        ...
        {activePet && !dashboard?.latestMilestone ? (
          <View className="rounded-[24rpx] bg-[#FFF5FA] p-5 mb-5 shadow-[0_14rpx_28rpx_rgba(190,98,144,0.08)]">
            ...
            <View
              className="mt-4 h-[74rpx] px-4 rounded-[18rpx] bg-white inline-flex items-center"
              onClick={() =>
                Taro.navigateTo({
                  url: `/pages/PetMilestones/index${activePetId ? `?petId=${activePetId}` : ''}`,
                })

...
const sourceActionMap: Record<
  TimelineSourceType,
  { title: string; url: (petId: string, date: string) => string }
> = {
  schedule: {
    title: '补提醒',
      ...
      <View className="px-6 pt-4 pb-[110rpx]">
          ...
          <View className="rounded-[24rpx] bg-[#F6F9FF] p-5 mb-5 shadow-[0_14rpx_28rpx_rgba(0,0,0,0.05)]">
            ...
            <View className="grid grid-cols-3 gap-3 mt-4">
              {actionItems.map((item) => (
                ...
                >
                  <Text
                    className="text-[24rpx] font-semibold"
                    style={{ color: item.textColor }}
                  >
                    {item.title}
                  </Text>
                  <Text
                    className="text-[20rpx] mt-[6rpx] block"
                    style={{ color: item.textColor, opacity: 0.82 }}
                  >
                    {item.desc}
                  </Text>
                </View>
        ...
        <View className="grid grid-cols-4 gap-3 mb-5">
          <View className="rounded-[18rpx] bg-white p-4 shadow-[0_12rpx_22rpx_rgba(0,0,0,0.05)]">
            ...
            <Text className="text-[30rpx] font-semibold text-[#2c2c2c] mt-[6rpx] block">
              {summary.schedule}
            </Text>
          </View>
          <View className="rounded-[18rpx] bg-white p-4 shadow-[0_12rpx_22rpx_rgba(0,0,0,0.05)]">
        ...
        {!loggedIn ? (
          <View className="rounded-[24rpx] bg-white p-5 shadow-[0_14rpx_28rpx_rgba(0,0,0,0.06)]">
            ...
            <View
              ...
              onClick={() =>
                ensureLoggedIn(
                  `/pages/PetTimeline/index${currentPetId ? `?petId=${currentPetId}` : ''}`
                )
              }
            >
          ...
          groupedTimeline.map((group) => (
            <View key={group.date} className="mb-5">
              <View className="flex items-center mb-3">
                ...
                <Text className="text-[26rpx] font-semibold text-[#2c2c2c]">
                  {group.date}
                </Text>
              </View>
          ...
          <View className="rounded-[24rpx] bg-white p-5 shadow-[0_14rpx_28rpx_rgba(0,0,0,0.06)]">
            <Text className="text-[24rpx] text-[#8a8a8a] leading-[1.7]">
              {activePet?.name || '这个宠物'}
              在当前筛选下还没有足够的时间线数据，先去补提醒、记录、花销、护理、食物、用药或里程碑吧。
            </Text>
            {currentPetId ? (
              <View className="grid grid-cols-2 gap-3 mt-4">
                ...
                >
                  <Text className="text-[24rpx] font-semibold text-[#2c5f7a]">
                    新增记录
                  </Text>
                ...
                <View
                  ...
                  onClick={() =>
                    Taro.navigateTo({
                      url: `/pages/PetMilestones/index?petId=${currentPetId}`,
                    })
                  }
                >
                  <Text className="text-[24rpx] font-semibold text-[#8A5374]">
                    记录成长里程碑
                  </Text>

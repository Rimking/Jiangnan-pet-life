  ...
  return (
    ...
    >
      <View className="px-8 pt-4 pb-[120px]">
        <View className="mb-3">
          <Text className="text-[26px] text-[#555]">宠物：{pet.name}</Text>
        </View>
        ...
        <View className="flex flex-wrap gap-2 mb-4">
          {monthOptions.map((item) => (
            ...
            >
              <Text className="text-[22px]">{item.label}</Text>
            </View>
          ))}
        </View>
        ...
        >
          <Text className="text-[28px] font-bold">当月总花销：¥{summary.total.toFixed(2)}</Text>
          <Text className="text-[22px] text-[#666] mt-1 block">记录笔数：{summary.count}</Text>
        </View>
        ...
        <View
          ...
          style={{ boxShadow: PET_UI_SHADOW }}
        >
          <Text className="text-[30px] font-bold mb-3 block">分类占比</Text>
          {summary.byCategory.length ? (
              ...
              return (
                <View key={item.name} className="mb-3">
                  <View className="flex items-center justify-between mb-1">
                    ...
                    <Text className="text-[24px] text-[#666]">¥{item.amount.toFixed(2)}</Text>
                  </View>
            ...
            <Text className="text-[24px] text-[#8a8a8a]">本月暂无花销记录</Text>
          )}
        </View>
        ...
        <View
          ...
          style={{ boxShadow: PET_UI_SHADOW }}
        >
          <Text className="text-[30px] font-bold mb-3 block">花销明细</Text>
          {monthlyExpenses.length ? (
            monthlyExpenses.map((item) => (
              <View
                key={item.id}
                className="mb-2 p-3 rounded-[12px] border-[2px] border-solid border-[#262626] bg-white flex items-center justify-between"
              >
                <View className="flex-1">
                  <Text className="text-[24px] block">{item.category}</Text>
                  <Text className="text-[20px] text-[#7a7a7a]">{item.date} {item.time}</Text>
                </View>
                <View className="items-end">
                  <Text className="text-[24px] text-[#ff6b6b]">¥{item.amount.toFixed(2)}</Text>
                  {item.note ? <Text className="text-[20px] text-[#9a9a9a]">{item.note}</Text> : null}
                </View>
              </View>
            ))
          ) : (
            <Text className="text-[24px] text-[#8a8a8a]">本月暂无明细</Text>
          )}
        </View>
      </View>
    </BasicLayout>

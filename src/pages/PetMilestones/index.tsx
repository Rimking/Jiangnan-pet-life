    ...
    >
      <View className="px-6 pt-4 pb-[120rpx]">
        {!loggedIn ? (
          <View className="mb-5 rounded-[20rpx] bg-[#FFF7D5] p-4">
            ...
            <View
              ...
              onClick={() =>
                ensureLoggedIn(`/pages/PetMilestones/index${preferredPetId ? `?petId=${preferredPetId}` : ''}`)
              }
            >
        ...
        <View className="rounded-[24rpx] bg-white p-5 mb-5 shadow-[0_14rpx_30rpx_rgba(0,0,0,0.08)]">
          ...
          <Text className="text-[22rpx] text-[#666] mt-[8rpx] block">当前已记录 {milestones.length} 个里程碑</Text>
          {latestMilestone ? (
            <Text className="text-[22rpx] text-[#666] mt-[6rpx] block">
              最近一次：{latestMilestone.title} · {latestMilestone.occurredAt.slice(0, 10)}
            </Text>
          ) : (
            <Text className="text-[22rpx] text-[#888] mt-[6rpx] block">还没有成长记录，可以先补一个重要节点。</Text>
          )}
        </View>
        ...
        <View className="grid grid-cols-3 gap-3 mb-5">
          <View className="rounded-[20rpx] bg-white p-4 shadow-[0_14rpx_30rpx_rgba(0,0,0,0.06)]">
            ...
            <Text className="text-[34rpx] font-semibold text-[#2B2B2B] mt-[6rpx] block">{milestones.length}</Text>
          </View>
          <View className="rounded-[20rpx] bg-white p-4 shadow-[0_14rpx_30rpx_rgba(0,0,0,0.06)]">
            <Text className="text-[22rpx] text-[#666] block">标签数量</Text>
            <Text className="text-[34rpx] font-semibold text-[#2B2B2B] mt-[6rpx] block">{tagCount}</Text>
          </View>
          <View className="rounded-[20rpx] bg-white p-4 shadow-[0_14rpx_30rpx_rgba(0,0,0,0.06)]">
        ...
        <View className="rounded-[24rpx] bg-white p-5 mb-5 shadow-[0_14rpx_30rpx_rgba(0,0,0,0.08)]">
          <View className="mb-4 flex items-center justify-between">
            <Text className="text-[28rpx] font-semibold block">{editingId ? '编辑里程碑' : '新增里程碑'}</Text>
            {editingId ? (
              ...
              <View key={item.id} className="mb-3 rounded-[18rpx] bg-[#FFFBEA] p-4">
                <View className="flex justify-between items-start">
                  <View className="flex-1 pr-4">
                    <Text className="text-[28rpx] font-semibold block text-[#2B2B2B]">{item.title}</Text>

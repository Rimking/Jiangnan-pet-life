...
const PetCareStats = memo(function PetCareStats() {
  ...
  const careTypes = useMemo(() => {
    const types = Array.from(new Set(careList.map((item) => item.careType).filter(Boolean)));
  ...
  const upcomingCount = careList.filter((item) => item.nextDate && item.nextDate >= today).length;
  const overdueCount = careList.filter((item) => item.nextDate && item.nextDate < today).length;
    ...
    >
      <View className="px-6 pt-4 pb-[120rpx]">
        ...
        {!loggedIn ? (
          ...
          >
            <Text className="text-[30rpx] font-semibold text-[#2B2B2B] block">登录后查看护理档案</Text>
        ...
        {loggedIn && !petId ? (
          <View
            ...
            style={{ boxShadow: PET_UI_SHADOW }}
          >
            <Text className="text-[30rpx] font-semibold text-[#2B2B2B] block">还没有宠物档案</Text>
            <Text className="text-[22rpx] text-[#666] mt-2 block">
        ...
        >
          ...
          <Text className="text-[22rpx] text-[#666] mt-2 block">本月护理 {monthCount} 次</Text>
          <Text className="text-[22rpx] text-[#666] mt-1 block">累计记录 {careList.length} 条</Text>
          {recentCare ? (
            <Text className="text-[22rpx] text-[#666] mt-1 block">
              最近一次：{recentCare.careType} · {recentCare.date}
            </Text>
          ) : (
            <Text className="text-[22rpx] text-[#888] mt-1 block">还没有护理记录，可以先补一条最近完成的事项。</Text>
          )}
        </View>
        ...
        <View className="grid grid-cols-3 gap-3 mb-5">
          ...
          >
            ...
            <Text className="text-[34rpx] font-semibold text-[#2B2B2B] mt-1 block">{upcomingCount}</Text>
          </View>
          <View
            className="rounded-[20rpx] border-[3rpx] border-solid border-[#262626] bg-white p-4"
            style={{ boxShadow: PET_UI_SHADOW }}
          >
            ...
            <Text className="text-[34rpx] font-semibold text-[#D65A31] mt-1 block">{overdueCount}</Text>
          </View>
          <View
            className="rounded-[20rpx] border-[3rpx] border-solid border-[#262626] bg-white p-4"
            style={{ boxShadow: PET_UI_SHADOW }}
        ...
        <View className="mb-5">
          <Text className="text-[24rpx] font-semibold text-[#2B2B2B] mb-3 block">按护理类型筛选</Text>
        ...
        >
          <View className="mb-4 flex items-center justify-between">
            <Text className="text-[30rpx] font-semibold text-[#2B2B2B] block">护理时间线</Text>
            <Text className="text-[20rpx] text-[#7A7A7A]">{filteredCareList.length} 条记录</Text>
          </View>
          ...
          {filteredCareList.length ? (
            filteredCareList.map((item, index) => {
              ...
              return (
                <View key={item.id} className={index === filteredCareList.length - 1 ? 'flex gap-3' : 'flex gap-3 mb-4'}>
                  <View className="w-[16rpx] flex flex-col items-center pt-2">
                    ...
                    {index !== filteredCareList.length - 1 ? <View className="w-[2rpx] flex-1 bg-[#B7B7B7] mt-1" /> : null}
                  </View>

                  <View className="flex-1 rounded-[16rpx] border-[2rpx] border-solid border-[#262626] bg-white p-4">
                    <View className="flex items-start justify-between gap-3">
                      <View className="flex-1">
                        <Text className="text-[26rpx] font-semibold text-[#2B2B2B] block">{item.careType}</Text>
                    ...
                    <Text className="text-[22rpx] text-[#4F4F4F] mt-3 block">结果：{item.result || '已完成'}</Text>
                    {item.note ? (
                      <Text className="text-[21rpx] text-[#7A7A7A] mt-2 block">备注：{item.note}</Text>
                    ) : null}
                    ...
                    <View className="mt-3 rounded-[14rpx] bg-[#F8F8F8] p-3">
                      ...
                      <Text className="text-[21rpx] mt-1 block" style={{ color: tone.color }}>
                        提醒状态：{status.label}
                      </Text>
                    </View>
                  </View>
